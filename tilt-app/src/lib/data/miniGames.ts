...
export class QuizGenerator {
	static generateRangeQuiz(position: Position, difficulty = 'easy'): QuizQuestion {
	...
	static generatePushFoldQuiz(stackSize: string, difficulty = 'medium'): QuizQuestion {
	...
	static generateExploitQuiz(difficulty = 'hard'): QuizQuestion {
	...
	static generateMixedQuiz(count = 10, difficulty = 'mixed'): QuizQuestion[] {
...
/**
 * Speed Drill Generator - fast yes/no decisions
 */
export class SpeedDrillGenerator {
	static generateHandDecision(position: Position, timeLimit = 5): SpeedDrill {
		const range = RFI_RANGES[position];
		const allHands = [
			'AA',
			'KK',
	...
	static generateSpeedDrill(position: Position, count = 20, timeLimit = 5): SpeedDrill[] {
		const drills: SpeedDrill[] = [];
		for (let i = 0; i < count; i++) {
			drills.push(SpeedDrillGenerator.generateHandDecision(position, timeLimit));
		}
		return drills;
	}
}
...
/**
 * Interactive Scenario - complex decision trees
 */
export class ScenarioGenerator {
	static generateLimperScenario(): Scenario {
